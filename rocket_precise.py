...
precision = int(input('How many decimal places would you like to calculate out to? (int): '))
decimal.getcontext().prec = precision  # Set the precision to 50 decimal places or adjust as needed    

g0 = decimal.Decimal('9.80665') # g0 : Acceleration due to gravity (m/s^2)
R = decimal.Decimal('287.05') # R : Specific gas constant for dry air (J/(kg·K))
T0 = decimal.Decimal('288.15') # T0 : Standard temperature at sea level (K)
L = decimal.Decimal('0.0065') # L : Temperature lapse rate (K/m)
P0 = decimal.Decimal('101325.0') # P0 : Standard atmospheric pressure at sea level (Pa)
gamma = decimal.Decimal('1.4') # gamma : Specific heat ratio for dry air

area = decimal.Decimal(input("Cross-sectional area (m²): "))
mass = decimal.Decimal(input("Rocket mass (kg): "))
impulse = decimal.Decimal(input("Mass-Specific impule (s): "))
thrust = decimal.Decimal(input("Thrust duration (s): "))
...
time_step = decimal.Decimal('0.01')  # s
end_time = thrust_duration * decimal.Decimal('100.0')  # s
...
def atmosphere(altitude):
    altitude = decimal.Decimal(altitude)  # Ensure altitude is a Decimal
    pressure = decimal.Decimal(P0 * (1 - L * altitude / T0) ** (g0 / (R * L)))
    temperature = decimal.Decimal(T0 - L * altitude)
...
def gravity(altitude):
    altitude = decimal.Decimal(altitude)  # Ensure altitude is a Decimal
    return g0 + g0 / ((decimal.Decimal('1') + altitude / decimal.Decimal('6.371e6')) ** decimal.Decimal('2'))


# Function to calculate speed of sound as a function of altitude
def speed_of_sound(altitude, temperature):
    temperature = decimal.Decimal(temperature)  # Ensure temperature is a Decimal
    return decimal.Decimal(math.sqrt(gamma * R * abs(temperature)))

def drag_coefficient(mach):
    return decimal.Decimal('0.2') + decimal.Decimal('0.1') * decimal.Decimal(math.exp(decimal.Decimal('-0.12') * mach))
...
def rocket_acceleration(y, t, thrust_function, drag_coefficient, rocket_mass):
    ...
    if t >= thrust_duration:
        ...
        # Thrust
        thrust = decimal.Decimal(thrust_function(t))

    # Convert velocity to Decimal
    velocity = decimal.Decimal(velocity)

    # Calculate Mach number
    mach = velocity / speed_of_sound(altitude, temperature)

    # Calculate drag
    drag = decimal.Decimal('0.5') * density * velocity**2 * A * drag_coefficient(mach)
    ...
    if thrust > 0.0:
        rocket_mass = decimal.Decimal(mass)
...
if profile == '1':
    # Constant Thrust (ideal rocket)
    max_thrust = decimal.Decimal(input("Enter the constant thrust value (N): "))
    thrust_profile = constant_thrust

elif profile == '2':
    # Single Stage (Linear Decrease in Thrust)
    max_thrust = decimal.Decimal(input("Enter the initial thrust value (N): "))
    thrust_rate = decimal.Decimal(input("Enter the thrust rate (N/s): "))
    thrust_profile = linear_decrease_thrust

elif profile == '3':
    # Multiple Stages (Stepwise Thrust Profile)
    max_thrust_stage1 = decimal.Decimal(input("Enter the thrust value for stage 1 (N): "))
    max_thrust_stage2 = decimal.Decimal(input("Enter the thrust value for stage 2 (N): "))
    stage1_burn_time = decimal.Decimal(input("Enter the burn time for stage 1 (s): "))
    stage2_burn_time = decimal.Decimal(input("Enter the burn time for stage 2 (s): "))
    thrust_profile = stepwise_thrust

elif profile == '4':
    # Periodic Burst (Thrust Pulse Profile)
    max_thrust_pulse = decimal.Decimal(input("Enter the thrust value during pulses (N): "))
    pulse_duration = decimal.Decimal(input("Enter the duration of each thrust pulse (s): "))
    pulse_period = decimal.Decimal(input("Enter the time between thrust pulses (s): "))
